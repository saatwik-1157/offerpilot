...
(function () {
  ...
  function exportCsv() {
    if (!_apps.length) { window.toast("No applications to export yet"); return; }
    var rows = [["Company", "Role", "Location", "Match %", "Status"]];
    _apps.slice().sort(function (a, b) { return b.day - a.day; }).forEach(function (a) {
      rows.push([a.company, a.role, a.location, a.matchScore, a.status]);
    });
    var csv = rows.map(function (r) { return r.map(csvCell).join(","); }).join("\n");
    download("offerpilot-applications.csv", csv, "text/csv;charset=utf-8");
    window.toast("Exported " + _apps.length + " applications");
  }
  ...
  function renderAppTable() {
    ...
    var list = _apps.filter(function (a) {
      if (filter !== "All" && a.status !== filter) return false;
      if (q && (a.company + " " + a.role).toLowerCase().indexOf(q) === -1) return false;
      return true;
    });
    ...
    document.getElementById("app-count").textContent =
      (filter === "All" && !q) ? _apps.length + " total" : list.length + " of " + _apps.length;
  ...
  function resumeHTML(app, r) {
    ...
    return '<div class="resume-doc">' +
        ...
        '<div style="margin-top:18px;display:flex;align-items:center;gap:10px;flex-wrap:wrap">' +
          ...
          '<button class="btn btn-ghost btn-sm" id="resume-download">⬇ Download .txt</button>' +
  ...
  function renderResume(app, r) {
    ...
    body.querySelector("#resume-download").addEventListener("click", function () {
      var txt = (r.role || "") + " — " + app.company + "\n" +
        "Match: " + app.matchScore + "%\n\n" +
        (r.summary || "") + "\n\n" +
        "Keywords: " + (r.keywords || []).join(", ") + "\n\nHighlights:\n" +
        (r.highlights || []).map(function (h) { return "- " + h; }).join("\n") +
        "\n\n— Generated by OfferPilot RezForge™";
      download("resume-" + app.company.replace(/[^A-Za-z0-9]/g, "") + ".txt", txt);
      window.toast("Resume downloaded");
    });
  ...
  if (searchEl) searchEl.addEventListener("input", renderAppTable);
  var exportEl = document.getElementById("app-export");
